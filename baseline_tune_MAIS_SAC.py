...
# ------------------ SET SAC STABLE-BASELINE AGENT HYPERPARAMETERS ------------------
...
# ------------------ SET MULTI AGENT HYPERPARAMETERS ------------------
# default hyperparameter values remain unchanged.
multi_agent_kwargs = {
    'lr': 0.0006,
    'batch_size': 128,
    'replay_buffer_capacity': 1000000,
    # 'standardize_start_time_step': 100
    'tau': 0.005,
...
###########Decentralized-independent SAC not using stable baseline


#######Decentralized-dependent information sharing SAC e##############
from citylearn.agents.marlisa import MARLISA as MySACAgent_multi
my_sac_env = CityLearnEnv(    
    DATASET_NAME,
    central_agent=False,
    buildings=BUILDINGS,
    active_observations=ACTIVE_OBSERVATIONS,
...
# wrap environment
my_sac_env = NormalizedObservationWrapper(my_sac_env)
my_sac_model = MySACAgent_multi(my_sac_env, **multi_agent_kwargs)
# my_sac_model = MySACAgent_multi(my_sac_env)
...
# observations, _ = my_sac_env.reset()
my_sac_actions_list = []

my_sac_model.env.unwrapped.evaluate()
